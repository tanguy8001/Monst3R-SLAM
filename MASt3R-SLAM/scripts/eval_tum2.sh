...
if [ "$print_only" = false ]; then
    for dataset in ${datasets[@]}; do
        ...
        if [ "$no_calib" = true ]; then
            python main.py --dataset $dataset_name --no-viz --save-as tum2/no_calib/$dataset --config config/eval_no_calib.yaml
        else
            python main.py --dataset $dataset_name --no-viz --save-as tum2/calib/$dataset --config config/eval_calib.yaml
        fi
    done
fi
